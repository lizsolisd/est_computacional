...
```{r}
#con gather apilas columnas - alargo los datos para q cada variable sea una columna
#y cada observ sea un renglón
#nombre de las nvas variables (income, frequency)
#q columna no apilar -nombre de var
...
ggplot(pew_tidy_2, aes(x = income, y = percent, group = religion)) +
  facet_wrap(~ religion, nrow = 1) + #para hacer varias gráficas en el mismo plot
  #aqui esta separado por la religion con un solo renglon
  geom_bar(stat = "identity", fill = "darkgray") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
#angle es para rotar los labesl del eje x
```
...
billboard
#info desde q entró al billboard como fue rankeado x semana
```
...
#apliar: wk1:wk76
#data= billboard
#nombre nvas variables: week, rank
#na.rm quita los NAs - dependiendo del problema te conviene o quitar los NA's
...
```{r}
#parse_number de la variable saco unicamente los numeros x ej: week2 solo se queda con 2


billboard_tidy <- billboard_long %>%
  mutate(
    week = parse_number(week),
    date = date.entered + 7 * (week - 1), #hago la fecha que entraron +7 x la semana para q obtenga la fecha del ranking
...
ggplot(tracks, aes(x = date, y = rank)) +
  geom_line() + 
  facet_wrap(~track, nrow = 1) + #para graficar todo en el mismo renglón
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
#nombre de la col q va a contener las variables q pasaras a renglones (nombre nvas variables: week, rank)
#na.rm quita los NAs - dependiendo del problema te conviene o quitar los NA's
#gather(nombre del df, nombre de la nueva variable que esta en columnas y pasaras a una sola columna, valor de la variable nueva)
# ** contains
# ** separate con la posición

gather(data = tb, sexo_edad_nombre, sexo_edad_valor, new_sp_m04:new_sp_fu, na.rm = TRUE) %>% 
  mutate(edad = parse_number(sexo_edad_nombre))

gather(data = tb, sexo_edad_nombre, sexo_edad_valor, contains("new"), na.rm = TRUE)
...
#el 8 te dice donde cortar una var se queda antes del 8 y la segunda dps del 8 y dependiendo del numero 
#de cortes es el nombre de vars q debes hacer
...
separate(tb_long, demo, c("parte1", "parte2", "parte3"), sep = "_")
```
...
- cada observacione tiene dos renglones: 1 para t max y otra para t minima
...
clima_vars <- clima_long %>% 
  mutate(day = parse_number(day), #del caracter extrae el número 
    value = as.numeric(value) / 10) %>% #convertimos a ºC
...
#1 df
#2do variable q vamos a alrgar
#3ª columna cuyos valores vana  estra abajo de element
clima_tidy
```
...
##fullnames = TRUE es que quiero toda la ruta y no solo el nombre del archivos
##pattern el patron
...
```{r}
#es un vector que no tiene nombres
...
```{r, error=TRUE}
# q quiero hacer?? leer un csv
# Tddi es time double double integer y va en orden
file1 <- read_csv(paths[1], col_types = "Tddi")
file1

##Generalizando esto xq los quieor para los 330 csvs
##map_df map lo aplica la funcion 
...
#Modelo y detalles del avion
planes <- read_csv("data/planes.csv")
planes

#Info de aeropuertos de destino houston
...
*NOTA*: Debemos tener cuidado al usar `==`
...
unique(flights$dest) %>% 
  sort()
#Sort para ordenar vector y arrange una base de datos
